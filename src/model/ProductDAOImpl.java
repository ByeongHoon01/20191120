...
public class ProductDAOImpl extends DAOBase implements ProductDAO {
	...
	public ArrayList<ProductVO> readPProductList() throws SQLException {
		...
		try {
			conn = getConnection();
			sql = "SELECT pname, (pnum - jnum) FROM PRODUCT where jnum < pnum*0.2 ";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				do {
					ProductVO pdt = new ProductVO();
					pdt.setPname(rs.getString(1));
					pdt.setPnum(rs.getInt(2));
					products.add(pdt);
	...
	public ArrayList<ProductVO> readProfitList() throws SQLException {
		...
		try {
			conn = getConnection();
			sql = "SELECT PNAME, JNUM*(SALE - COST) FROM PRODUCT order by jnum * (sale - cost) desc";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				do {
